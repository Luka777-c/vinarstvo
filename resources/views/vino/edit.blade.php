        ...
        <div class="max-w-4xl mx-auto">
            ...
            <div class="flex justify-between items-center mb-10">
                <div class="flex items-center gap-4">
                    ...
                    <h1 class="text-5xl font-bold text-gray-800">Izmena vina</h1>
                </div>
            ...
            <form action="{{ route('vino.update', $vino->id) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="bg-[#F2EBE1] border border-stone-400 rounded-3xl overflow-hidden shadow-sm mb-12">
                    ...
                    <div class="flex items-center p-6 border-b border-stone-400">
                        ...
                        <input type="text" name="naziv" value="{{ $vino->naziv }}" required 
                    ...
                    <div class="flex items-center p-6 border-b border-stone-400">
                        ...
                        <select name="tip" required class="w-full bg-white border border-stone-300 rounded-md p-2 outline-none">
                            <option value="Crveno" {{ $vino->tip == 'Crveno' ? 'selected' : '' }}>Crveno</option>
                            <option value="Belo" {{ $vino->tip == 'Belo' ? 'selected' : '' }}>Belo</option>
                            <option value="Rose" {{ $vino->tip == 'Rose' ? 'selected' : '' }}>Rose</option>
                        </select>
                    </div>

                    <div class="flex items-center p-6 border-b border-stone-400">
                        <label class="text-2xl font-medium text-gray-700 w-1/3">Količina (kom/L):</label>
                        <input type="number" name="kolicina" value="{{ $vino->kolicina }}" required 
                               class="w-full bg-white border border-stone-300 rounded-md p-2 outline-none">
                    </div>

                    <div class="flex items-center p-6 border-b border-stone-400">
                        <label class="text-2xl font-medium text-gray-700 w-1/3">Datum proizvodnje:</label>
                        <input type="date" name="datum_proizvodnje" value="{{ $vino->datum_proizvodnje->format('Y-m-d') }}" required 
                               class="w-full bg-white border border-stone-300 rounded-md p-2 outline-none">
                    </div>
                    ...
                    <div class="flex items-center p-6 border-b border-stone-400">
                        ...
                        <select name="partija_grozdja_id" required class="w-full bg-white border border-stone-300 rounded-md p-2 outline-none">
                            @foreach($partije as $p)
                                <option value="{{ $p->id }}" {{ $vino->partija_grozdja_id == $p->id ? 'selected' : '' }}>
                                    {{ $p->sorta }} ({{ $p->datum->format('Y-m-d') }})
                                </option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="flex items-center p-6">
                        ...
                        <select name="bure_id" class="w-full bg-white border border-stone-300 rounded-md p-2 outline-none">
                            @foreach($burad as $b)
                                <option value="{{ $b->id }}" {{ $vino->bure_id == $b->id ? 'selected' : '' }}>
                                    Bure {{ $b->broj_bureta }}
                                </option>
                            @endforeach
                        </select>
