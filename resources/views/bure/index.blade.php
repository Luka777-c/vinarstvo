        ...
        <div class="max-w-5xl mx-auto">
            ...
            <div class="flex justify-between items-start mb-10">
                <div class="flex items-center gap-4">
                    ...
                    <h1 class="text-5xl font-bold text-gray-800">Skladište / Burad</h1>
                </div>
                <div class="flex gap-3">
                    <a href="{{ route('bure.create') }}" class="px-6 py-2 bg-[#D49A3D] text-white font-bold rounded-md shadow-sm hover:opacity-90">+ Nova</a>
                    <a href="/" class="px-6 py-2 bg-[#4A1D1D] text-white font-bold rounded-md shadow-sm hover:opacity-90">Nazad</a>
                </div>
            </div>

            <div class="mb-8 flex items-center gap-4">
                <span class="text-2xl font-medium text-gray-700">Pretraga po vinu / broju buradi:</span>
                ...
                <table class="w-full text-left border-collapse">
                    <thead class="bg-gray-50 border-b border-gray-200">
                        <tr class="text-gray-600 font-bold uppercase text-sm italic">
                            <th class="p-4">ID</th>
                            <th class="p-4">Broj buradi</th>
                            <th class="p-4">Tip drveta</th>
                            <th class="p-4">Kapacitet</th>
                    ...
                    <tbody class="text-gray-700">
                        ...
                        <tr class="border-b border-gray-100 hover:bg-gray-50 transition">
                            <td class="p-4">{{ str_pad($b->id, 2, '0', STR_PAD_LEFT) }}</td>
                            <td class="p-4">{{ $b->broj_bureta }}</td>
                            <td class="p-4">{{ $b->tip_drveta }}</td>
                            <td class="p-4">{{ $b->kapacitet }} L</td>
                            <td class="p-4 font-semibold {{ $b->status == 'prazno' ? 'text-gray-400' : 'text-[#D49A3D]' }}">
                                {{ $b->status }}
                            </td>
                            <td class="p-4 text-right">
                                <div class="flex justify-end items-center gap-4">
                                    <a href="{{ route('bure.show', $b->id) }}" class="text-[#D49A3D] underline font-medium hover:text-[#4A1D1D] transition">Details</a>
                                    
                                    <a href="{{ route('bure.edit', $b->id) }}" class="text-blue-600 hover:text-blue-800 transition" title="Izmeni">
                                        <i class="fa-solid fa-pen-to-square"></i>
                                    </a>

                                    <form action="{{ route('bure.destroy', $b->id) }}" method="POST" onsubmit="return confirm('Da li ste sigurni da želite da obrišete ovo bure?')">
                                        @csrf
                                        @method('DELETE')
