        ...
        <div class="max-w-4xl mx-auto">
            ...
            <div class="flex justify-between items-center mb-10">
                <div class="flex items-center gap-4">
                    ...
                    <h1 class="text-5xl font-bold text-gray-800 italic">Izmena bure</h1>
                </div>
            ...
            <form action="{{ route('bure.update', $bure->id) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="bg-[#F2EBE1] border border-stone-400 rounded-3xl overflow-hidden shadow-sm mb-12">
                    ...
                    <div class="flex items-center p-6 border-b border-stone-400">
                        ...
                        <input type="text" name="broj_bureta" value="{{ $bure->broj_bureta }}" required class="w-full bg-white border border-stone-300 rounded-md p-2 outline-none focus:ring-1 focus:ring-[#4A1D1D]">
                    </div>

                    <div class="flex items-center p-6 border-b border-stone-400">
                        <label class="text-2xl font-medium text-gray-700 w-1/3">Kapacitet (L):</label>
                        <input type="number" name="kapacitet" value="{{ $bure->kapacitet }}" required class="w-full bg-white border border-stone-300 rounded-md p-2 outline-none focus:ring-1 focus:ring-[#4A1D1D]">
                    </div>
                    ...
                    <div class="flex items-center p-6 border-b border-stone-400">
                        ...
                        <select name="tip_drveta" required class="w-full bg-white border border-stone-300 rounded-md p-2 outline-none focus:ring-1 focus:ring-[#4A1D1D]">
                            <option value="Hrast" {{ $bure->tip_drveta == 'Hrast' ? 'selected' : '' }}>Hrast</option>
                            <option value="Bagrem" {{ $bure->tip_drveta == 'Bagrem' ? 'selected' : '' }}>Bagrem</option>
                            <option value="Inox" {{ $bure->tip_drveta == 'Inox' ? 'selected' : '' }}>Inox</option>
                        </select>
                    </div>
                    ...
                    <div class="flex items-center p-6 border-b border-stone-400">
                        ...
                        <select name="status" required class="w-full bg-white border border-stone-300 rounded-md p-2 outline-none focus:ring-1 focus:ring-[#4A1D1D]">
                            <option value="prazno" {{ $bure->status == 'prazno' ? 'selected' : '' }}>Prazno</option>
                            <option value="ciscenje" {{ $bure->status == 'ciscenje' ? 'selected' : '' }}>Ciscenje</option>
                            <option value="puno" {{ $bure->status == 'puno' ? 'selected' : '' }}>Puno</option>
                        </select>
                    </div>

                    <div class="flex items-center p-6">
                        <label class="text-2xl font-medium text-gray-700 w-1/3">Napomena:</label>
                        <input type="text" name="napomena" value="{{ $bure->napomena }}" class="w-full bg-white border border-stone-300 rounded-md p-2 outline-none focus:ring-1 focus:ring-[#4A1D1D]">
                    </div>
